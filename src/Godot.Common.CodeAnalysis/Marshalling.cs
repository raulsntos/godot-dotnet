...
internal static class Marshalling
{
    public static bool TryGetMarshallingInformation(Compilation compilation, ITypeSymbol typeSymbol, [MaybeNullWhen(false)] out MarshalInfo marshalInfo)
    {
        ...
        string fullyQualifiedTypeName = typeSymbol.FullQualifiedNameWithGlobal();
    ...
    private static bool TryGetVariantTypeForCoreTypes(ITypeSymbol typeSymbol, out VariantType variantType, out VariantTypeMetadata variantTypeMetadata)
    {
        ...
        (VariantType? maybeVariantType, variantTypeMetadata) = typeSymbol.SpecialType switch
        {
            ...
            _ when typeSymbol.FullQualifiedNameOmitGlobal() == KnownTypeNames.SystemHalf => (VariantType.Float, VariantTypeMetadata.None),
            ...
            _ when typeSymbol.FullQualifiedNameOmitGlobal() == KnownTypeNames.SystemTextRune => (VariantType.Int, VariantTypeMetadata.Char32),
    ...
    public static bool TryGetPackedArrayType(ITypeSymbol typeSymbol, ITypeSymbol elementTypeSymbol, out VariantType packedArrayType, [NotNullWhen(true)] out string? fullyQualifiedMarshalAsTypeName)
    {
        (packedArrayType, fullyQualifiedMarshalAsTypeName) = elementTypeSymbol.SpecialType switch
        {
            ...
            SpecialType.System_Single => (VariantType.PackedFloat32Array, KnownTypeNames.GodotPackedFloat32Array),
        ...
        if (fullyQualifiedMarshalAsTypeName is not null)
        {
            return true;
        }

        (packedArrayType, fullyQualifiedMarshalAsTypeName) = elementTypeSymbol.FullQualifiedNameOmitGlobal() switch
        {
            ...
            KnownTypeNames.GodotColor => (VariantType.PackedColorArray, KnownTypeNames.GodotPackedColorArray),
            KnownTypeNames.GodotVector4 => (VariantType.PackedVector4Array, KnownTypeNames.GodotPackedVector4Array),

            _ => (VariantType.Nil, null),
        };

        return fullyQualifiedMarshalAsTypeName is not null;
    }

    public static bool TryGetArrayLikeElementType(Compilation compilation, ITypeSymbol arrayLikeTypeSymbol, [NotNullWhen(true)] out ITypeSymbol? elementTypeSymbol)
    {
    ...
    public static bool TryGetDictionaryLikeKeyValueTypes(Compilation compilation, ITypeSymbol dictionaryLikeTypeSymbol, [NotNullWhen(true)] out ITypeSymbol? keyTypeSymbol, [NotNullWhen(true)] out ITypeSymbol? valueTypeSymbol)
    {
        keyTypeSymbol = null;
    ...
    private static void GetEnumPropertyHint(ITypeSymbol enumTypeSymbol, out PropertyHint hint, out string? hintString)
    {
        ...
        bool hasFlagsAttr = enumTypeSymbol.HasAttribute(KnownTypeNames.SystemFlagsAttribute);
    ...
    {
        return $"global::{KnownTypeNames.GodotArray}<{elementTypeSymbol.FullQualifiedNameWithGlobal()}>";
    }

    private static string MakeGenericGodotDictionary(ITypeSymbol keyTypeSymbol, ITypeSymbol valueTypeSymbol)
    {
        return $"global::{KnownTypeNames.GodotDictionary}<{keyTypeSymbol.FullQualifiedNameWithGlobal()}, {valueTypeSymbol.FullQualifiedNameWithGlobal()}>";
    }
}
