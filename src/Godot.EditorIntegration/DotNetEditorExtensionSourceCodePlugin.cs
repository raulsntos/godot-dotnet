...
internal sealed partial class DotNetEditorExtensionSourceCodePlugin : EditorExtensionSourceCodePlugin, IDisposable
{
    ...
    protected override StringName _GetClassNameFromSourcePath(string sourcePath)
    {
        ...
        static async Task<StringName> GetClassNameFromSourcePathCoreAsync(string sourcePath, CancellationToken cancellationToken = default)
        {
            ...
            List<INamedTypeSymbol> typeSymbolsDeclaredInDocument = [];
            ...
            foreach (var classDeclarationSyntax in classDeclarationSyntaxes)
            {
                ...
                typeSymbolsDeclaredInDocument.Add(symbol);
            }

            if (typeSymbolsDeclaredInDocument.Count == 0)
            {
                return StringName.Empty;
            }

            if (typeSymbolsDeclaredInDocument.Count > 1)
            {
                GD.PushWarning($"Found more than one class declaration in file '{sourcePath}', selecting the first one.");
            }

            return new StringName(typeSymbolsDeclaredInDocument[0].Name);
        }
    }
    ...
    protected override Error _CreateClassSource(string className, string baseClassName, PackedStringArray paths)
    {
        ...
        return _CreateClassSourceFromTemplateId(className, baseClassName, paths, nameof(Templates.DefaultTemplateId), []);
    }
