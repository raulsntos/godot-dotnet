...
partial class PackedScene
{
    ...
    public T Instantiate<T>(GenEditState editState = 0) where T : class
    {
    ...
    /// <typeparam name="T">The type to cast to. Should be a descendant of <see cref="Node"/>.</typeparam>
    /// <returns>The instantiated scene.</returns>
    public T? InstantiateOrNull<T>(GenEditState editState = 0) where T : class
    {
