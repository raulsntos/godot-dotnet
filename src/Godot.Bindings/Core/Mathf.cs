...
public static partial class Mathf
{
    ...
    /// </summary>
    public const real_t Tau = real_t.Tau;
    ...
    /// </summary>
    public const real_t Pi = real_t.Pi;
    ...
    {
        return double.Clamp(value, min, max);
    }
    ...
    public static float CubicInterpolateInTime(float from, float to, float pre, float post, float weight, float toT, float preT, float postT)
    {
        ...
        float a3 = float.Lerp(to, post, postT - toT == 0 ? 1.0f : (t - toT) / (postT - toT));
        float b1 = float.Lerp(a1, a2, toT - preT == 0 ? 0.0f : (t - preT) / (toT - preT));
        float b2 = float.Lerp(a2, a3, postT == 0 ? 1.0f : t / postT);
        return float.Lerp(b1, b2, toT == 0 ? 0.5f : t / toT);
    }
    ...
    public static double CubicInterpolateInTime(double from, double to, double pre, double post, double weight, double toT, double preT, double postT)
    {
        ...
        double a3 = double.Lerp(to, post, postT - toT == 0 ? 1.0 : (t - toT) / (postT - toT));
        double b1 = double.Lerp(a1, a2, toT - preT == 0 ? 0.0 : (t - preT) / (toT - preT));
        double b2 = double.Lerp(a2, a3, postT == 0 ? 1.0 : t / postT);
        return double.Lerp(b1, b2, toT == 0 ? 0.5 : t / toT);
    }
    ...
    {
        return float.Exp(db * 0.11512925464970228420089957273422f);
    }
    ...
    {
        return double.Exp(db * 0.11512925464970228420089957273422);
    }
    ...
    public static float Ease(float s, float curve)
    {
        ...
        if (curve > 0.0f)
        {
            if (curve < 1.0f)
            {
                return 1.0f - float.Pow(1.0f - s, 1.0f / curve);
            }

            return float.Pow(s, curve);
        }
        ...
        if (curve < 0.0f)
        {
            if (s < 0.5f)
            {
                return float.Pow(s * 2.0f, -curve) * 0.5f;
            }

            return ((1.0f - float.Pow(1.0f - ((s - 0.5f) * 2.0f), -curve)) * 0.5f) + 0.5f;
        }
    ...
    public static double Ease(double s, double curve)
    {
        ...
        if (curve > 0)
        {
            if (curve < 1.0)
            {
                return 1.0 - double.Pow(1.0 - s, 1.0 / curve);
            }

            return double.Pow(s, curve);
        }
        ...
        if (curve < 0.0)
        {
            if (s < 0.5)
            {
                return double.Pow(s * 2.0, -curve) * 0.5;
            }

            return ((1.0 - double.Pow(1.0 - ((s - 0.5) * 2.0), -curve)) * 0.5) + 0.5;
        }
    ...
    {
        return float.Log(linear) * 8.6858896380650365530225783783321f;
    }
    ...
    {
        return double.Log(linear) * 8.6858896380650365530225783783321;
    }
    ...
    {
        if (float.Abs(to - from) <= delta)
        {
            return to;
        }

        return from + (float.Sign(to - from) * delta);
    }
    ...
    {
        if (double.Abs(to - from) <= delta)
        {
            return to;
        }

        return from + (double.Sign(to - from) * delta);
    }
    ...
    {
        return float.Lerp(outFrom, outTo, InverseLerp(inFrom, inTo, value));
    }
    ...
    {
        return double.Lerp(outFrom, outTo, InverseLerp(inFrom, inTo, value));
    }

    /// <summary>
    /// Rotates <paramref name="from"/> toward <paramref name="to"/> by the <paramref name="delta"/> amount.
    /// Will not go past <paramref name="to"/>.
    /// Similar to <see cref="MoveToward(float, float, float)"/> but interpolates correctly when
    /// the angles wrap around <see cref="Tau"/>.
    ...
    public static float RotateToward(float from, float to, float delta)
    {
        ...
        float absDifference = float.Abs(difference);
        return from + float.Clamp(delta, absDifference - float.Pi, absDifference) * (difference >= 0.0f ? 1.0f : -1.0f);
    }

    /// <summary>
    /// Rotates <paramref name="from"/> toward <paramref name="to"/> by the <paramref name="delta"/> amount.
    /// Will not go past <paramref name="to"/>.
    /// Similar to <see cref="MoveToward(double, double, double)"/> but interpolates correctly when
    /// the angles wrap around <see cref="Tau"/>.
    /// If <paramref name="delta"/> is negative, this function will rotate away from <paramref name="to"/>,
    /// toward the opposite angle, and will not go past the opposite angle.
    /// </summary>
    ...
    public static double RotateToward(double from, double to, double delta)
    {
        ...
        double absDifference = double.Abs(difference);
        return from + double.Clamp(delta, absDifference - double.Pi, absDifference) * (difference >= 0.0 ? 1.0 : -1.0);
    }
    ...
    public static float SmoothStep(float from, float to, float weight)
    {
        ...
        float x = float.Clamp((weight - from) / (to - from), 0.0f, 1.0f);
        return x * x * (3 - (2 * x));
    }
    ...
    public static double SmoothStep(double from, double to, double weight)
    {
        ...
        double x = double.Clamp((weight - from) / (to - from), 0.0, 1.0);
        return x * x * (3 - (2 * x));
    }
    ...
    public static double Snapped(double s, double step)
    {
        if (step != 0f)
        {
            return double.Floor((s / step) + 0.5) * step;
        }
    ...
    {
        return (length != 0.0f) ? float.Abs(Fract((value - length) / (length * 2.0f)) * length * 2.0f - length) : 0.0f;
    ...
    {
        return (length != 0.0) ? double.Abs(Fract((value - length) / (length * 2.0)) * length * 2.0 - length) : 0.0;

        static double Fract(double value)
        {
            return value - double.Floor(value);
        }
    }
}
