...
public struct Vector3 : IEquatable<Vector3>
{
    ...
    internal void Normalize()
    {
        ...
        {
            real_t length = real_t.Sqrt(lengthsq);
            X /= length;
            Y /= length;
            Z /= length;
        }
    ...
    /// </summary>
    /// <returns>A vector with <see cref="real_t.Abs(real_t)"/> called on each component.</returns>
    ...
    {
        return real_t.Atan2(Cross(to).Length(), Dot(to));
    }
    ...
    /// </summary>
    /// <returns>A vector with <see cref="real_t.Ceiling(real_t)"/> called on each component.</returns>
    ...
    /// </summary>
    /// <returns>A vector with <see cref="real_t.Floor(real_t)"/> called on each component.</returns>
    ...
    public readonly Vector3 Lerp(Vector3 to, real_t weight)
    {
        return new Vector3
        (
            real_t.Lerp(X, to.X, weight),
            real_t.Lerp(Y, to.Y, weight),
            real_t.Lerp(Z, to.Z, weight)
        );
    }
    ...
    /// new vector will be <see cref="real_t.NaN"/>.
    /// </summary>
    ...
    {
        return new Vector3(real_t.Round(X), real_t.Round(Y), real_t.Round(Z));
    }
    ...
    public readonly real_t SignedAngleTo(Vector3 to, Vector3 axis)
    {
        ...
        real_t unsignedAngle = real_t.Atan2(crossTo.Length(), Dot(to));
    ...
    public readonly Vector3 Slerp(Vector3 to, real_t weight)
    {
        ...
        {
            // Colinear vectors have no rotation axis or angle between them, so the best we cacn do is lerp.
            return Lerp(to, weight);
        }
        axis /= real_t.Sqrt(axisLengthSquared);
        real_t startLength = real_t.Sqrt(startLengthSquared);
        real_t resultLength = real_t.Lerp(startLength, real_t.Sqrt(endLengthSquared), weight);
    ...
    {
        Vector3 n = this;
        n /= real_t.Abs(n.X) + real_t.Abs(n.Y) + real_t.Abs(n.Z);
        Vector2 o;
        if (n.Z >= 0.0f)
        {
            o.X = n.X;
        ...
        {
            o.X = (1.0f - real_t.Abs(n.Y)) * (n.X >= 0.0f ? 1.0f : -1.0f);
            o.Y = (1.0f - real_t.Abs(n.X)) * (n.Y >= 0.0f ? 1.0f : -1.0f);
        }
        o.X = o.X * 0.5f + 0.5f;
        o.Y = o.Y * 0.5f + 0.5f;
        return o;
    ...
    /// <summary>
    /// Returns the Vector3 from an octahedral-compressed form created using <see cref="OctahedronEncode()"/>
    /// (stored as a Vector2).
    /// </summary>
    ...
    {
        Vector2 f = new Vector2(oct.X * 2.0f - 1.0f, oct.Y * 2.0f - 1.0f);
        Vector3 n = new Vector3(f.X, f.Y, 1.0f - real_t.Abs(f.X) - real_t.Abs(f.Y));
        real_t t = real_t.Clamp(-n.Z, 0.0f, 1.0f);
        n.X += n.X >= 0 ? -t : t;
        n.Y += n.Y >= 0 ? -t : t;
        return n.Normalized();
    }
