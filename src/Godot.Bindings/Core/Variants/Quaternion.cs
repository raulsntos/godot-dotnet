...
public struct Quaternion : IEquatable<Quaternion>
{
    ...
    {
        real_t dot = Dot(to);
        return real_t.Acos(real_t.Clamp(dot * dot * 2 - 1, -1, 1));
    }
    ...
    public readonly Quaternion SphericalCubicInterpolate(Quaternion b, Quaternion preA, Quaternion postB, real_t weight)
    {
        ...
        if (!IsNormalized())
        {
            throw new InvalidOperationException(SR.InvalidOperation_QuaternionIsNotNormalized);
        }
        if (!b.IsNormalized())
        {
            throw new ArgumentException(SR.Argument_ValueIsNotNormalized, nameof(b));
        }
#endif
        ...
        bool flip1 = real_t.Sign(fromQ.Dot(preQ)) < 0;
        preQ = flip1 ? -preQ : preQ;
        bool flip2 = real_t.Sign(fromQ.Dot(toQ)) < 0;
        toQ = flip2 ? -toQ : toQ;
        bool flip3 = flip2 ? toQ.Dot(postQ) <= 0 : real_t.Sign(toQ.Dot(postQ)) < 0;
    ...
    {
        if (real_t.Abs(W) > 1 - Mathf.Epsilon)
        {
            return new Vector3(X, Y, Z);
        }

        real_t r = 1 / real_t.Sqrt(1 - W * W);
    ...
    {
        return real_t.IsFinite(X) && real_t.IsFinite(Y) && real_t.IsFinite(Z) && real_t.IsFinite(W);
    }
    ...
    /// part (<see cref="X"/>, <see cref="Y"/>, and <see cref="Z"/>). The returned
    /// quaternion's real part (<see cref="W"/>) is always <c>0.0</c>.
    /// </summary>
    ...
    {
        return real_t.Sqrt(LengthSquared());
    }
    ...
    public readonly Quaternion Slerp(Quaternion to, real_t weight)
    {
        ...
        if (!to.IsNormalized())
        {
            throw new ArgumentException(SR.Argument_ValueIsNotNormalized, nameof(to));
        }
#endif
        ...
        {
            // Standard case (Slerp).
            real_t omega = real_t.Acos(cosom);
            sinom = real_t.Sin(omega);
            scale0 = real_t.Sin((1.0f - weight) * omega) / sinom;
            scale1 = real_t.Sin(weight * omega) / sinom;
        }
        else
        {
    ...
    public readonly Quaternion Slerpni(Quaternion to, real_t weight)
    {
        ...
        if (real_t.Abs(dot) > 0.9999f)
        {
            return this;
        }

        real_t theta = real_t.Acos(dot);
        real_t sinT = 1.0f / real_t.Sin(theta);
        real_t newFactor = real_t.Sin(weight * theta) * sinT;
        real_t invFactor = real_t.Sin((1.0f - weight) * theta) * sinT;
    ...
    public Quaternion(Vector3 axis, real_t angle)
    {
        ...
        {
            (real_t sin, real_t cos) = real_t.SinCos(angle * 0.5f);
            real_t s = sin / d;
    ...
    /// <summary>
    /// Constructs a <see cref="Quaternion"/> representing the shortest arc between
    ...
    public static Quaternion FromEuler(Vector3 eulerYXZ)
    {
        ...
        (real_t sinA1, real_t cosA1) = real_t.SinCos(halfA1);
        (real_t sinA2, real_t cosA2) = real_t.SinCos(halfA2);
        (real_t sinA3, real_t cosA3) = real_t.SinCos(halfA3);
    ...
    public static Vector3 operator *(Quaternion quaternion, Vector3 vector)
    {
        ...
        {
            throw new ArgumentException(SR.Argument_ValueIsNotNormalized, nameof(quaternion));
        }
#endif
