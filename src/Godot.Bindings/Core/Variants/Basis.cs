...
public struct Basis : IEquatable<Basis>
{
        ...
        switch (order)
        {
                ...
                if (sy < (1.0f - Mathf.Epsilon))
                {
                    if (sy > -(1.0f - Mathf.Epsilon))
                    {
                        ...
                        if (Row1[0] == 0 && Row0[1] == 0 && Row1[2] == 0 && Row2[1] == 0 && Row1[1] == 1)
                        {
                            ...
                            euler.X = 0;
                            euler.Y = real_t.Atan2(Row0[2], Row0[0]);
                            euler.Z = 0;
                        }
                        else
                        {
                            euler.X = real_t.Atan2(-Row1[2], Row2[2]);
                            euler.Y = real_t.Asin(sy);
                            euler.Z = real_t.Atan2(-Row0[1], Row0[0]);
                        }
                    }
                    else
                    {
                        euler.X = real_t.Atan2(Row2[1], Row1[1]);
                        euler.Y = -real_t.Tau / 4.0f;
                        euler.Z = 0.0f;
                    }
                }
                else
            ...
            case EulerOrder.Yxz:
            {
                ...
                if (m12 < (1 - Mathf.Epsilon))
                {
                    if (m12 > -(1 - Mathf.Epsilon))
                    {
                        ...
                        {
                            // return the simplest form (human friendlier in editor and scripts)
                            euler.X = real_t.Atan2(-m12, Row1[1]);
                            euler.Y = 0;
                            euler.Z = 0;
                        }
                        else
                        {
                            euler.X = real_t.Asin(-m12);
                            euler.Y = real_t.Atan2(Row0[2], Row2[2]);
                            euler.Z = real_t.Atan2(Row1[0], Row1[1]);
                        }
                    }
                    else
                    { // m12 == -1
                        euler.X = real_t.Tau / 4.0f;
                        euler.Y = real_t.Atan2(Row0[1], Row0[0]);
                        euler.Z = 0;
                    }
                }
                else
            ...
            case EulerOrder.Yzx:
            {
                ...
                if (sz < (1.0f - Mathf.Epsilon))
                {
                    if (sz > -(1.0f - Mathf.Epsilon))
                    {
                        euler.X = real_t.Atan2(-Row1[2], Row1[1]);
                        euler.Y = real_t.Atan2(-Row2[0], Row0[0]);
                        euler.Z = real_t.Asin(sz);
                    }
                    else
                    {
                        // It's -1
                        euler.X = real_t.Atan2(Row2[1], Row2[2]);
                        euler.Y = 0.0f;
                        euler.Z = -real_t.Tau / 4.0f;
                    }
                }
            ...
            case EulerOrder.Zxy:
            {
                ...
                if (sx < (1.0f - Mathf.Epsilon))
                {
                    if (sx > -(1.0f - Mathf.Epsilon))
                    {
                        euler.X = real_t.Asin(sx);
                        euler.Y = real_t.Atan2(-Row2[0], Row2[2]);
                        euler.Z = real_t.Atan2(-Row0[1], Row1[1]);
                    }
                    else
                    {
                        // It's -1
            ...
            case EulerOrder.Zyx:
            {
                ...
                if (sy < (1.0f - Mathf.Epsilon))
                {
                    if (sy > -(1.0f - Mathf.Epsilon))
                    {
                        euler.X = real_t.Atan2(Row2[1], Row2[2]);
                        euler.Y = real_t.Asin(-sy);
                        euler.Z = real_t.Atan2(Row1[0], Row0[0]);
                    }
                    else
                    {
                        // It's -1
    ...
    internal readonly Quaternion GetQuaternion()
    {
        ...
        if (trace > 0.0f)
        {
            real_t s = real_t.Sqrt(trace + 1.0f) * 2f;
            real_t inv_s = 1f / s;
        ...
        {
            real_t s = real_t.Sqrt(Row0[0] - Row1[1] - Row2[2] + 1.0f) * 2f;
            real_t inv_s = 1f / s;
            return new Quaternion
            (
                s * 0.25f,
    ...
    public readonly Basis Slerp(Basis target, real_t weight)
    {
        ...
        b.Row0 *= real_t.Lerp(Row0.Length(), target.Row0.Length(), weight);
        b.Row1 *= real_t.Lerp(Row1.Length(), target.Row1.Length(), weight);
        b.Row2 *= real_t.Lerp(Row2.Length(), target.Row2.Length(), weight);
    ...
    {
        (real_t sin, real_t cos) = real_t.SinCos(euler.X);
    ...
    /// See <see cref="Transposed"/>.
    /// For transforming by inverse of a non-orthonormal basis (e.g. with scaling)
    /// <c>basis.Inverse() * vector</c> can be used instead. See <see cref="Inverse"/>.
    /// </summary>
    ...
    {
        return Row0.IsEqualApprox(other.Row0)
            && Row1.IsEqualApprox(other.Row1)
