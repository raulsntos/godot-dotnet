...
public struct Transform2D : IEquatable<Transform2D>
{
    ...
    /// </summary>
    public readonly real_t Rotation => real_t.Atan2(X.Y, X.X);
    ...
    public readonly Transform2D Orthonormalized()
    {
        ...
        orthoX.Normalize();
        orthoY -= orthoX * orthoX.Dot(orthoY);
        orthoY.Normalize();
    ...
    {
        return this with { Origin = Origin + offset };
    }
    ...
    {
        return this with { Origin = Origin + BasisXform(offset) };
    }
    ...
    {
        (real_t sin, real_t cos) = real_t.SinCos(rotation);
        X.X = Y.Y = cos;
        X.Y = Y.X = sin;
        Y.X *= -1;
        Origin = origin;
    ...
    {
        (real_t rotationSin, real_t rotationCos) = real_t.SinCos(rotation);
        (real_t rotationSkewSin, real_t rotationSkewCos) = real_t.SinCos(rotation + skew);
    ...
    public static Vector2 operator *(Transform2D transform, Vector2 vector)
    {
        return new Vector2
        (
            transform.Tdotx(vector),
            transform.Tdoty(vector)
        ) + transform.Origin;
    }
    ...
    public static Vector2 operator *(Vector2 vector, Transform2D transform)
    {
        ...
        return new Vector2
        (
            transform.X.Dot(vInv),
            transform.Y.Dot(vInv)
        );
    }
