...
public struct Color : IEquatable<Color>
{
    ...
    public int R8
    {
        readonly get => (int)float.Round(R * 255.0f);
        set => R = value / 255.0f;
    }
    ...
    public int G8
    {
        readonly get => (int)float.Round(G * 255.0f);
        set => G = value / 255.0f;
    }
    ...
    public int B8
    {
        readonly get => (int)float.Round(B * 255.0f);
        set => B = value / 255.0f;
    }
    ...
    public int A8
    {
        readonly get => (int)float.Round(A * 255.0f);
        set => A = value / 255.0f;
    }
    ...
    /// </summary>
    /// <value>Getting is equivalent to using <see cref="float.Max(float, float)"/> on the RGB components. Setting uses <see cref="FromHsv"/>.</value>
    public float V
    {
        readonly get => float.Max(R, float.Max(G, B));
    ...
    /// using <see cref="float.Clamp(float, float, float)"/>.
    /// </summary>
    ...
    public readonly Color Clamp(Color? min = null, Color? max = null)
    {
        ...
        return new Color
        (
            float.Clamp(R, minimum.R, maximum.R),
            float.Clamp(G, minimum.G, maximum.G),
            float.Clamp(B, minimum.B, maximum.B),
            float.Clamp(A, minimum.A, maximum.A)
        );
    }
    ...
    public readonly Color Lerp(Color to, real_t weight)
    {
        return new Color
        (
            float.Lerp(R, to.R, weight),
            float.Lerp(G, to.G, weight),
            float.Lerp(B, to.B, weight),
            float.Lerp(A, to.A, weight)
        );
    }
    ...
    {
        uint c = (byte)float.Round(A * byte.MaxValue);
        c <<= 8;
        c |= (byte)float.Round(B * byte.MaxValue);
        c <<= 8;
        c |= (byte)float.Round(G * byte.MaxValue);
        c <<= 8;
        c |= (byte)float.Round(R * byte.MaxValue);
    ...
    {
        ulong c = (ushort)float.Round(A * ushort.MaxValue);
        c <<= 16;
        c |= (ushort)float.Round(B * ushort.MaxValue);
        c <<= 16;
        c |= (ushort)float.Round(G * ushort.MaxValue);
        c <<= 16;
        c |= (ushort)float.Round(R * ushort.MaxValue);
    ...
    {
        uint c = (byte)float.Round(R * byte.MaxValue);
        c <<= 8;
        c |= (byte)float.Round(G * byte.MaxValue);
        c <<= 8;
        c |= (byte)float.Round(B * byte.MaxValue);
        c <<= 8;
        c |= (byte)float.Round(A * byte.MaxValue);
    ...
    {
        ulong c = (ushort)float.Round(R * ushort.MaxValue);
        c <<= 16;
        c |= (ushort)float.Round(G * ushort.MaxValue);
        c <<= 16;
        c |= (ushort)float.Round(B * ushort.MaxValue);
        c <<= 16;
        c |= (ushort)float.Round(A * ushort.MaxValue);
    ...
    {
        return $"{ToHex32(R)}{ToHex32(G)}{ToHex32(B)}{(includeAlpha ? ToHex32(A) : "")}";
    }
    ...
    /// <see cref="NamedColors"/> constants.
    /// </summary>
    ...
    /// color names are the same as the <see cref="NamedColors"/> constants.
    /// </summary>
    /// <param name="code">The HTML color code or color name to construct from.</param>
    ...
    /// the constants defined in <see cref="NamedColors"/>.
    /// </summary>
    ...
    private static bool FindNamedColor(string name, out Color color)
    {
        ...
        return NamedColors.ByName.TryGetValue(sb.ToString(), out color);
    }
    ...
    public static Color FromRgbe9995(uint rgbe)
    {
        ...
        float e = rgbe >> 27;
        float m = float.Pow(2.0f, e - 15.0f - 9.0f);
    ...
    {
        byte b = (byte)float.Round(float.Clamp(val * 255, 0, 255), MidpointRounding.AwayFromZero);
        return $"{b:x2}";
    }
    ...
    {
        return NamedColors.White - color;
    }
