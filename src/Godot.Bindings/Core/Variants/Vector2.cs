...
public struct Vector2 : IEquatable<Vector2>
{
    ...
    ///
    /// Equivalent to the result of <see cref="real_t.Atan2(real_t, real_t)"/> when
    ...
    {
        return real_t.Atan2(Cross(to), Dot(to));
    }
    ...
    {
        return real_t.Atan2(to.Y - Y, to.X - X);
    }
    ...
    {
        return real_t.Sqrt((X - to.X) * (X - to.X) + (Y - to.Y) * (Y - to.Y));
    }
    ...
    {
        return new Vector2(real_t.Floor(X), real_t.Floor(Y));
    }
    ...
    {
        return real_t.IsFinite(X) && real_t.IsFinite(Y);
    }
    ...
    {
        return new Vector2(real_t.Round(X), real_t.Round(Y));
    }
